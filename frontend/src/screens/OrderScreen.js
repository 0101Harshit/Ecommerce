  ...
  useEffect(() => {
    ...
    if (!order._id || successPay || successDeliver || (order._id && order._id !== orderId)) {
      ...
      const loadPaypalScript = async () => {
        const { data: clientId } = await axios.get('/api/key/paypal/', { headers: { authorization: `Bearer ${userInfo.token}` } })
        paypalDispatch({
          type: 'resetOptions',
          value: {
            'client-id': clientId,
              ...
              <ListGroup variant="flush">
                ...
                {userInfo.isAdmin && order.isPaid && !order.isDelivered && (<ListGroup.Item>
                  ...
                  <div className='d-grid'>
                    <Button type="button" variant="warning" onClick={deliverOrderHandler}>
                      Deliver Order
                    </Button>
                  </div>
                </ListGroup.Item>)}
