  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const { data } = await axios.get(`/api/products/search?page=${page}&query=${query}&price=${price}&rating=${rating}&category=${category}&order=${order}`)
     ...
     fetchData();
  }, [category, error, order, query, price, rating, page])
      ...
      <Row>
        <Col md={3}>
          ...
          <div>
            ...
            <ul>
              <li>
                <Link className={price === 'all' ? 'text-bold' : ''}
            ...
            error ? (<MessageBox variant='danger'>{error}</MessageBox>) :
              (<>
                <Row className="justify-content-between mb-3">
                  <Col md={6}>
                    <div>
                      {!countProducts ? 'No' : countProducts} Results
                      {query !== 'all' && ' : ' + query}
